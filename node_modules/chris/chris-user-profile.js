...
class greenPageLoader extends PolymerElement {
    ...
    constructor(){
        ...
        // <meta property="dz:data-id" content="D41-269">
        this.user = store.get('subUser') || null;
        
        if (Dazzle.editMode==="normal"){
            if (!this.user) {
                alert('You have not yet logged in. Please come back later.');
                location.href="/index.html";
            }
        }
        
        this.wrapper = document.querySelector('[data-wrapper]')
        this.template = document.querySelector('template#chris-order-item').innerHTML;
        this.orderManager = new DataPackage('order');
        
        // Display User Value
        document.querySelectorAll('input').forEach(elm=>{
           let field = elm.getAttribute('name');
           elm.value = this.user[field];
        });
        ...
        let allHtml = '';
        this.total = 0;
        this.orderManager.searchDataByES(json).then(res=>{
            res.forEach(item=>{
               let html = this.orderManager.replaceToken(item,this.template); 
               allHtml +=html;
               this.total += res['total'];
            });    
            this.wrapper.innerHTML = allHtml;
            this.initPayment(res);
            this._listener();
        });
//        wrapper.innerHTML = this.orderManager.replaceToken(res,this.template);
        ...
        // this.template = document.querySelector('[data-wrapper]').innerHTML;
        // this.productManager.getDataByES(this.itemId).then(res=>{
        //   let html = this.productManager.replaceToken(res,this.template);
        //   wrapper.innerHTML = html;
        //   this._listener();
        // });
        // console.log('Page Loader',this.itemId);

    }
    initPayment(res){
                let paypal = window['paypal'];
            function initPayPalButton(json) {
                  paypal.Buttons({
                    style: {
                      shape: 'rect',
                      color: 'gold',
            ...
            let json = [
                    {
                        ...
                        "amount":{
                            ...
                            "breakdown":{
                                "item_total":{
                                        "currency_code":"HKD","value":res['total']
                                },
                                "shipping":{"currency_code":"HKD","value":0},
                                "tax_total":{"currency_code":"HKD","value":0}
                            }
                        }
                    }
                    ];
            console.log('Payment',json);
            initPayPalButton(json);
    }
    connectedCallback() {
